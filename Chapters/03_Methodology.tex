...
\begin{enumerate}
  ...
  \item \textbf{Service Layer}: Encapsulates cognitive capabilities in swappable interfaces:
        \begin{itemize}
          \item \texttt{PlanningService}: Daily planning and \emph{task} decomposition
...
The biggest change is the programmatic addition of movement \emph{actions}. In the original paper, characters moved as part of determined \emph{actions}; for example, if an agent planned to eat breakfast for 20 minutes in a coffee shop that is 25 minutes from their current location, they would walk to the café for 20 minutes and, without even reaching the café, would start another activity. In our opinion, this reduced the believability of the original approach, so we decided to automatically block time for the movement activity. This ensures that the agent always has time to reach the destination before starting \emph{actions}. However, it also requires estimating the time the agent will spend moving for each \emph{task} during planning, as the \emph{action} location is determined only after decomposing the \emph{task} into \emph{actions}.


\subsubsection{Daily Planning}

In the original paper, each \emph{task}, such as "morning routine," was always generated as a multiple of one hour. With newer models and enforced structured output, we were able to ask the model to generate arbitrarily long \emph{tasks} for a day. Another improvement is asking the model to generate \emph{task} intent, what type of \emph{actions} we expect to be necessary and the location of interest for the \emph{task}---the area where the agent will most likely move during \emph{task} execution.
We use \emph{task} intent as context for prompts that will operate on the \emph{task} [replanning, \emph{task} decomposition]. Locations of interest are used to estimate travel time during the activity.

\subsubsection{Task Decomposition}
When an agent approaches a \emph{task} that has not yet been decomposed, we trigger the \emph{task}'s decomposition into atomic \emph{actions}. In the original implementation, each \emph{action} was a multiple of 5 minutes, which produced some unbelievable \emph{actions}, such as washing hands for 5 minutes. In our implementation, we allow the model to arbitrarily choose the duration in minutes. If neuro-symbolic validation is enabled, we enter the validation loop. Then, with the final \emph{task} decomposition, we ask the model for the most appropriate place to perform each \emph{action}. We then programmatically generate movement \emph{actions} if the agent needs to switch locations between \emph{actions}. Finally, we check if the total time is above or below the expected time for the \emph{task}. We define the difference as delta [the time for the \emph{task} itself vs. the expected time for the movement]. If we need to fill time, we add a "Checking phone" activity for |delta| minutes. If delta > 0, we need to decrease the time spent on \emph{actions}. We achieve this by randomly decreasing the duration of activities by one minute, weighting activities that take longer so that a 20-minute activity has a ten times higher chance of being selected for decrease than a 2-minute activity [make the examples more mathematically strict].



\subsection{Neuro-Symbolic Validation Pipeline}
\label{subsec:neuro-symbolic-pipeline}
We implemented the following neuro-symbolic validation pipeline to enhance the coherence of \emph{actions} generated during \emph{task} decomposition. This is performed after generating \emph{actions} from the \emph{task}, but before adding movements. [TODO add example] We check the validity of preconditions using the external symbolic validation tool, VAL\cite{howey2004val}. To receive feedback from VAL, we must generate three files: Domain.pddl, Problem.pddl, and plan.pddl. Below, we explain how we achieved this.
...
We ask the model for natural language conditions and effects for each \emph{action} separately to prevent the chained condition-effect problem.
\begin{verbatim}
...
Now, for each \emph{action}, we generate an \emph{action} schema from the predicate catalog and natural language conditions and effects. We do this separately for each \emph{action} to avoid encouraging condition and effect chaining. Afterward, we programmatically create the domain file.
\begin{verbatim}
...
\end{verbatim}
If all conditions are satisfied, the agent will start the \emph{action}.
...
Both simulations featured the same three personas with backgrounds taken from the original study. We generated exactly the same goals and \emph{tasks} for the day [for each persona] for both simulations.
...
\begin{itemize}
  ...
  \item \textbf{H2 (action believability)}: For the same scenario, participants flag fewer \emph{actions} as ``unbelievable'' in our method than in the baseline.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Ours (Neuro-symbolic)}: Proposed system with symbolic planning and consistency checks integrated into deliberation and \emph{action} selection.
\end{enumerate}
...
The stimulus for each condition is a replay of a single random character's day. To focus on \emph{action} believability, we present:
...
\begin{itemize}
  ...
  \item UI controls to mark an \emph{action} as unbelievable (``thumbs down''), provide a short reason, and continue.
\end{itemize}

Replays cover the same scenario (e.g., two simulated in-game days) and use the same character profile, \emph{task} for the day and randomness seed across conditions, so any variation is attributable to agent architecture (baseline vs. ours) rather than scenario noise.
...
\begin{enumerate}
  ...
  \item \textbf{Condition A.} Watch the replay, freely scrub, and mark unbelievable \emph{actions}. For each mark, add a short explanation (optional but encouraged).
  \item \textbf{Condition B.} Repeat with the other planner. Order varies across participants; assignment is double-blind.
  \item \textbf{Summary.} We show the user an overview of marked \emph{actions} in both conditions and ask which one felt more human-like.
\end{enumerate}
...
\begin{figure}[H]
  ...
  \caption{User interface for the study, showing the replay view and mechanism for flagging unbelievable \emph{actions}.}
