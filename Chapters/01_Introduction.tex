...
This study builds on the generative agents architecture \cite{parkGenerativeAgentsInteractive2023a} by replacing the hierarchical planning component with a neuro-symbolic planning framework. The LLM generates PDDL action schemas and plans, which are then validated by a symbolic checker that detects constraint violations. The approach is evaluated through two methods:
...
\begin{itemize}
    ...
    \item \textbf{Qualitative evaluation}: Perceived believability is assessed via a within-subjects user study comparing agent behaviors from both systems.
\end{itemize}
