...
Current LLM-driven agents sacrifice either flexibility (over-reliance on symbolic systems) or logical consistency (purely neural models). 
...
\textbf{Aim:} To develop and evaluate a hybrid neuro-symbolic planning system in which an LLM generates PDDL-based plans and a symbolic validator verifies logical consistency and identifies constraint violations.
...
This study extends the generative agents architecture \cite{parkGenerativeAgentsInteractive2023a} by replacing hierarchical planning with a neuro-symbolic framework. The LLM generates PDDL action schemas and plans; a symbolic validator detects constraint violations. Evaluation combines:
