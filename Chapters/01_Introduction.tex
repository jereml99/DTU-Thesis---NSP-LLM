...
Neuro-symbolic AI can address this by combining neural generation with symbolic planning \cite{tantakounLLMsPlanningModelers2025}. % Should we call it plannning or verification
...
This study extends the generative agents architecture \cite{parkGenerativeAgentsInteractive2023a} by replacing hierarchical planning with a neuro-symbolic framework. The LLM generates hierarchical plans and PDDL action schemas; a symbolic validator detects constraint violations. % Constrain violation is vague, we need some examples. also we generate not only action schemas but the full domain and problem files.
 Evaluation combines:
...
\begin{itemize}
    ...
    Metrics include violations per 100 actions, plan success rates, and repair efficiency. % Not really sure about this line 
\end{itemize}
...
% I don't like this section, it seems detached from our actuall thesis, and we don't have a deteriminnistis envirment. als o the real world robotics seems off in our context, let's focus on actuall limitaiont that we had
...
% seems good now but let's refin at the end
\begin{itemize}
