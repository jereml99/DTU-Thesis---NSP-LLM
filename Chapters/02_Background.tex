...
To ground our approach, we introduce the formal concepts underlying automated planning. Formally, planning is grounded in \emph{restricted state-transition systems} \cite{ghallab2004automated, bolanderGentleIntroductionEpistemic2017}. However, to enable efficient modeling and solving, we focus on the \emph{structured representation} known as STRIPS \cite{fikes1971strips}, which defines states and actions using logic rather than explicit graphs. We assume a deterministic, fully observable environment.

\begin{definition}[State]
    \label{def:state}
    A \emph{state} $s$ is a description of the environment at a specific point in time. In classical planning, a state is represented as a finite set of logical facts (ground atoms) that are true. By the Closed-World Assumption, any fact not present in the state is false.
\end{definition}

\begin{definition}[Action]
    \label{def:action}
    An \emph{action} $a$ defines a transition between states. It is characterized by:
    \begin{itemize}
        \item $\text{name}(a)$: A unique identifier (e.g., $\texttt{move(isabella, home, cafe)}$);
        \item $\text{pre}(a)$: \emph{Preconditions}—a set of facts that must be in the current state for the action to be applicable;
        \item $\text{eff}^{+}(a)$: \emph{Add effects}—facts that become true after execution;
        \item $\text{eff}^{-}(a)$: \emph{Delete effects}—facts that become false.
    \end{itemize}
    An action $a$ is applicable in state $s$ if $\text{pre}(a) \subseteq s$. It compactly represents a transition in the underlying system to a new state $s' = (s \setminus \text{eff}^{-}(a)) \cup \text{eff}^{+}(a)$.
\end{definition}

\begin{definition}[Classical Planning Problem]
    \label{def:planning-problem}
    A classical planning problem is a tuple $\Pi = \langle P, A, I, G \rangle$, where:
    \begin{itemize}
        \item $P$ is a finite set of all possible facts (propositions);
        \item $A$ is a finite set of actions;
        \item $I \subseteq P$ is the \emph{initial state};
        \item $G \subseteq P$ is the \emph{goal condition} (a set of facts that must be true).
    \end{itemize}
\end{definition}

\begin{definition}[Plan and Solution]
    \label{def:plan}
    A \emph{plan} is a sequence of actions $\pi = \langle a_1, \ldots, a_n \rangle$. A plan is a \emph{solution} if actions are sequentially applicable starting from $I$ and the final state satisfies all facts in $G$.
\end{definition}

\begin{example}[Isabella's Commute]
    \label{ex:simple-planning}
    Consider Isabella needing to open her cafe.

    \textbf{Facts}: $P = \{ \texttt{at(home)}, \texttt{at(cafe)}, \texttt{cafe-open} \}$.

    \textbf{Initial State}: $I = \{ \texttt{at(home)} \}$.
    \textbf{Goal}: $G = \{ \texttt{at(cafe)}, \texttt{cafe-open} \}$.

    \textbf{Actions}:
    \begin{itemize}
        \item $\texttt{drive-to-cafe}$:
              \begin{itemize}
                  \item Pre: $\{ \texttt{at(home)} \}$
                  \item Eff: Add $\{ \texttt{at(cafe)} \}$, Delete $\{ \texttt{at(home)} \}$
              \end{itemize}
        \item $\texttt{open-up}$:
              \begin{itemize}
                  \item Pre: $\{ \texttt{at(cafe)} \}$
                  \item Eff: Add $\{ \texttt{cafe-open} \}$
              \end{itemize}
    \end{itemize}

    \textbf{Valid Plan}: $\pi = \langle \texttt{drive-to-cafe}, \texttt{open-up} \rangle$.
\end{example}
...
\begin{definition}[PDDL Domain and Problem]
    ...
    \begin{itemize}
        ...
        \item \textbf{Action Schemas}: Parameterized operators with preconditions and effects (corresponding to the action model defined in Definition \ref{def:action}).
    \end{itemize}
...
% ADD LLM based planning, just add waht LLM planning is about
...
\begin{enumerate}
    ...
    \item \textbf{Believability and coherence should be assessed jointly}: constraint adherence is necessary for plausibility (coherence), but human evaluation is required to confirm realism (believability) \cite{parkGenerativeAgentsInteractive2023a, xiaoHowFarAre2024}. % look into into this more
\end{enumerate}
