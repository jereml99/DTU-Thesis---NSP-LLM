...
For a plan $\pi = \langle a_1, \ldots, a_n \rangle$ (a finite sequence of actions) and an initial state $s_0 \in I$, we define the \emph{execution} of $\pi$ from $s_0$ recursively as follows: let $s_0' = s_0$, and for $i = 1, \ldots, n$, let $s_i' = T(s_{i-1}', a_i)$. The plan $\pi$ is \emph{valid} for the planning problem if, for every initial state $s_0 \in I$, the execution reaches a goal state, i.e., $s_n' \in G$. A valid plan is called a \emph{solution} to the planning problem.
\end{definition}
...
\end{itemize}
These assumptions enable precise reasoning about plan validity and correctness. Relaxing them leads to more expressive planning models, which are discussed later in this thesis.
...
\begin{itemize}
    \item Omit necessary preconditions (e.g. talking to a person that is not there),
    \item Hallucinate actions or states not grounded in the environment \cite{xiaoHowFarAre2024}.
\end{itemize}

Park et al.'s Generative Agents exhibited emergent social behaviors but lacked constraint enforcement, relying on LLM prompt engineering to maintain temporal coherence heuristically \cite{parkGenerativeAgentsInteractive2023a}.
...
Formal HTN or temporal PDDL planners can validate such decompositions, ensuring high-level commitments refine into feasible, non-overlapping primitive \emph{actions}. In our running example, ``complete coursework this week'' might decompose into ``attend lectures,'' ``complete assignments,'' and ``study for exam,'' with temporal constraints preventing overlaps and ensuring deadlines are met.

\subsection{Plan Validation and the VAL Tool}

Across hierarchical and LLM-based planning, a critical challenge emerges: how do we verify that a proposed plan is logically correct before execution? Symbolic planners guarantee correctness by construction (plans are valid by design), but LLM-generated and manually authored plans require post-hoc verification.
...
For LLM-driven agents, VAL serves as an external symbolic oracle that enforces formal correctness. Unlike generic language understanding (which may miss subtle logical errors), VAL provides guarantee: a valid plan (in VAL's judgment) respects all domain constraints. This enforcement directly operationalizes the concept of \emph{coherence} defined earlier (\cref{def:coherence}): a plan that passes VAL validation is coherent with respect to the domain.

\textbf{Limitations}: VAL only validates against the given PDDL model. If the model is incomplete or misspecified, a plan may pass validation despite being problematic in practice. Additionally, VAL does not measure plan quality, plausibility, or believability—it enforces logical correctness only.
...
\begin{example}[Neuro-Symbolic Validation of Action Decomposition]
    ...
    The LLM receives a daily \emph{task} ``work café shift (09:00 to 12:00)'' and decomposes it into fine-grained \emph{actions}: ``walk to café,'' ``unlock door,'' ``turn on lights,'' ``serve customers,'' ``clean tables.'' The PDDL validator checks each \emph{action}:
    \begin{itemize}
