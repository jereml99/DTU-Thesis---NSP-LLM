...
\textbf{Key findings} demonstrate that layman (ambiguous) descriptions yield 2.35 times more distinct solvable schema sets than detailed expert descriptions (8039 vs.\ 3419 with 10 LLMs, no CP), reflecting diverse valid interpretations. With 10 LLM instances, the probability of generating at least one solvable schema set exceeds 95\%. Conformal Prediction (CP) filtering reduces combinations to 3.3\% of original (1051/31,483) while increasing the solvable ratio from 10.9\% to 23.0\%. Human evaluators ranked pipeline-generated plans (mean 2.97) significantly higher than Tree-of-Thought baselines (3.58), with gold standard at 1.79. The pipeline successfully solved the Sussman Anomaly (requiring interleaved subgoal handling), whereas direct LLM approaches (GLM, GPT-3.5, GPT-4o) failed by attempting linear goal ordering.
