...
\begin{example}[Running Example: Isabella Rodriguez (Cafe Owner)]
    ...
    % This below seems not needed
    A complete PDDL formulation of Isabella's domain, including predicates, action schemas, and a concrete problem instance, is provided in \cref{app:pddl-example}.
\end{example}
...
[Maybe other introduction more focus on the PDDL]
...
To enable symbolic planning, domains and problems must be specified formally. The Planning Domain Definition Language (PDDL) is the standard representation used by most automated planners. [any sources to back it up?]

\begin{definition}[PDDL Domain and Problem]
    \label{def:pddl}
    [More about the predicate, like what dose the examples actually means, and on zero arity]
...
% This one focuses on temporal coherence which is not focus of our thesis, let's focus on some illogical action sequences
\begin{example}[Coherence Violation]
    \label{ex:coherence-violation}
    In our running example (\cref{ex:running-student}), a coherent agent must not schedule overlapping commitments. A purely LLM-based planner might generate ``Close Hobbs Cafe at 8pm'' and ``Welcome guests for Valentine's Party at 8pm'' without detecting that she cannot effectively close the business while simultaneously hosting a party event that requires the space to be open (or if she schedules ``Shop at Market'' while ``Serving Lunch'' at the cafe).
\end{example}
...
\begin{itemize}
    ...
    \item \textbf{Structured output generation}: Producing formal notations (code, logical rules, task lists) when prompted\cite{StructuredModelOutputs}. % this is quite crucial for our method, would be nice to get a bit more into details with this one. maybe explaining https://github.com/mlc-ai/xgrammar  constrained decoding or whatever openAI use for that.
\end{itemize}
...
% we seems to miss the text for this maybe just say how the models be usedd of planning. but also we talk about it below in later sections
...
% This seems like it should be part of literature review right?
...
% maybe not needed here. let's add this papaer insted Zhou, Zhehua, Jiayang Song, Kunpeng Yao, Zhan Shu, and Lei Ma. “ISR-LLM: Iterative Self-Refined Large Language Model for Long-Horizon Sequential Task Planning.” arXiv:2308.13724. Preprint, arXiv, August 26, 2023. https://doi.org/10.48550/arXiv.2308.13724. that talks about iterative plan improvemnt and problem with domain generation, but the don't do it based on action as we. they first generate the domain and problem and the they generate plans based on this
...
\begin{enumerate}
    \item \textbf{LLM planning produces human-like behavior but lacks long-horizon coherence} due to missing symbolic grounding, context window limitations, and hallucination \cite{parkGenerativeAgentsInteractive2023a, xiaoHowFarAre2024}. % This one is to refine, we don't realy focus on the long-horizon coherence in our thesis. at least not with our method, we mainly chech the coherence on task level
