...
Compared to Park et al., these hybrid methods assume an explicit domain model and delegate feasibility checking to a solver, trading authoring cost for guarantees. This thesis follows the hybrid path but investigates PDDL schema generation by LLMs to reduce authoring cost while preserving symbolic validation.
...
This thesis investigates whether LLM-generated PDDL schemas combined with symbolic validation reduce logical inconsistencies and incoherent action sequences while maintaining naturalistic, context-aware behavior. Research questions are articulated in Chapter 1 and operationalized through the experimental design in Chapter 3.
