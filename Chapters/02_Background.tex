...
Hierarchical task networks (HTN) decompose high-level tasks into ordered or partially ordered subtasks until primitive actions are reached, using methods encoding admissible refinements and constraints \cite{erolUMCPSoundComplete, nauSHOP2HTNPlanning2003}. Hierarchy supports abstraction, reuse, and tractable search.\footnote{This thesis does not develop a formal HTN model, as neither the Generative Agents architecture nor our neuro-symbolic approach employs HTN's formal decomposition semantics. Instead, we leverage the intuition of hierarchical task refinement informally to motivate and structure our discussion of how LLM-proposed tasks are decomposed into validated actions.}
...
Informally, we envision high-level tasks being decomposed into sequences of primitive actions. In our running example, ``complete coursework this week'' might decompose into ``attend lectures,'' ``complete assignments,'' and ``study for exam.'' Our neuro-symbolic approach operationalizes this decomposition through LLM generation and constraint validation, rather than formal task network semantics.
...
\begin{itemize}
    \item \textbf{Initial state}: propositions that are true at the start;
