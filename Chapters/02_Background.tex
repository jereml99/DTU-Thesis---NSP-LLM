...
It defines core concepts (LLMs, agents, planning paradigms including PDDL), % repetition?
 reviews LLM-driven generative agents with emphasis on the
  seminal%would anybody ever use this word? 
   Generative Agents paper \cite{parkGenerativeAgentsInteractive2023a}, and examines hybrid neuro-symbolic approaches combining LLM flexibility with symbolic planning guarantees \cite{huangPlanningDarkLLMSymbolic2024, tantakounLLMsPlanningModelers2025}. We emphasize the challenge of maintaining coherence (adherence to environmental constraints) and believability (human-perceived realism) in LLM-driven agents.

\begin{example}[Running Example: Student NPC]
% Now when we have the actuall simulation let's just get one of our characters as running examople
\label{ex:running-student}
Throughout this chapter, we illustrate concepts using a running example: an NPC simulating a university student managing academic and social commitments. The student must coordinate coursework (attending lectures, completing assignments), part-time work (café shifts), and social activities (meeting friends, attending events) while respecting temporal constraints (no overlapping commitments), location constraints (cannot be in two places simultaneously), % We mention temporal and location constrains several times but it's not at all a part of our thesis. (in our envirment agent cannot be in two places at once of do to things becous of game enginge)
 and environmental rules (must be enrolled in a course to attend its lectures).
\end{example}
...
% Let's get example from our actuall experiments not make up one. right now we don't have a concept of feeling in our thesis 
...
\begin{itemize}
    ...
    \item \textbf{Structured output generation}: Producing formal notations (code, logical rules, task lists) when prompted. % maybe a litte bit more about this, or at least references to OpenAI structured output generatiopn. and othere items as well
\end{itemize}

However, LLMs have critical limitations:
% References 
\begin{itemize}
    \item \textbf{No logical guarantees}: They perform statistical inference, not deductive reasoning. An LLM can produce plausible-sounding but logically inconsistent outputs. % any referemces 
...
% We use the LLM on basicly every step, so for the validation pipeline and also for condition checking at beginning of an action, so the bellows is outdated
...
% good example but maybe sth closser to our experiments. also we mix here formal language and natural language a bit too much
...
% insted of solver let's write about the validatior here 
...
% we don't really focus on this kind of coherence in our thesis
\label{def:coherence}
...
Operationally, coherence is enforced through constraint adherence: satisfaction of action preconditions, absence of temporal overlaps, respect for resource limits, and compliance with environmental invariants. While believability literature uses ``coherence'' informally to denote plausible behavior, in this thesis we use it in the stricter sense of constraint satisfaction validated by symbolic planning tools.
...
% Epistemic plasuability also seems out of tuch with what we actually did in our thesis. Also it's kind of filled with asslicking to thomas don't you think so?
...
% Again it would be probably better to focus on the validator 
